...
$(document).ready(function() {
  ...
  function formatedNum(num) {
    let tmp = String(new Intl.NumberFormat('en').format(num));
    return tmp;
  }
  ...
  const renderTemplate = ({title, areaFrom, areaTo, floor, areaPrice}) => {
    ...
    const template = `<div class="section-table__row" data-title="${title}" data-area="${areaFrom}-${areaTo}" data-floor="${floor}" data-price="${newPriceFrom} - ${newPriceTo}"">
      ...
      <div class="section-table__price section-table__text">
        <div class="mob">
          From ${newPriceFrom} <br>
          To ${newPriceTo}
        </div>
        <div class="desk">
          ${newPriceFrom} - ${newPriceTo}
        </div>
      </div>
    </div>`;
  ...
  const filterItems = (arr, callback) => {
    ...
    if (isFilterArea) {
      if (currency == 'VND') {
        ...
        res = tmp.filter(item => (item.areaPrice * cursDol) >= filterInputPriceMin.val().split(',').join('') && (item.areaPrice * cursDol) <= filterInputPriceMax.val().split(',').join(''));
      } else if (currency == 'euro') {
        res = tmp.filter(item => (item.areaPrice * cursEur) >= filterInputPriceMin.val().split(',').join('') && (item.areaPrice * cursEur) <= filterInputPriceMax.val().split(',').join(''));
      }
    } else {
      if (currency == 'VND') {
        res = tmp.filter(item => (item.areaPrice * item.areaTo) >= filterInputPriceMin.val().split(',').join('') && (item.areaPrice * item.areaFrom) <= filterInputPriceMax.val().split(',').join(''));
      } else if (currency == 'dolar') {
        res = tmp.filter(item => (item.areaPrice * item.areaTo * cursDol) >= filterInputPriceMin.val().split(',').join('') && (item.areaPrice * item.areaFrom * cursDol) <= filterInputPriceMax.val().split(',').join(''));
      } else if (currency == 'euro') {
        res = tmp.filter(item => (item.areaPrice * item.areaTo * cursEur) >= filterInputPriceMin.val().split(',').join('') && (item.areaPrice * item.areaFrom * cursEur) <= filterInputPriceMax.val().split(',').join(''));  
      }
    }
